...
function App() {
  ...
  const [gradeOtimizada, setGradeOtimizada] = useState(() => 
    carregarDados('gradeOtimizada', null)
  )
  ...
  const [profissionaisEditaveis, setProfissionaisEditaveis] = useState(() => 
    carregarDados('profissionaisEditaveis', profissionais)
  )
  const [necessidadesEditaveis, setNecessidadesEditaveis] = useState(() => 
    carregarDados('necessidadesEditaveis', necessidades)
  )
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-gray-900">Sistema Terapêutico Inteligente</h1>
          ...
          <Badge variant="secondary" className="mt-2">
            🚀 Superando limitações do GPT com funcionalidades reais
          </Badge>
        </header>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                  ...
                  {metricas && (
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <Badge variant={metricas.eficiencia > 80 ? "default" : "destructive"}>
                          {metricas.eficiencia}%
                        </Badge>
                      </div>
                      <div className="flex justify-between">
          ...
          <TabsContent value="grade" className="space-y-6">
            <GradeEditavel 
              gradeInicial={gradeOtimizada} 
              profissionais={profissionaisEditaveis}
              onGradeChange={setGradeOtimizada}
            />
          </TabsContent>
                  ...
                  <CardContent>
                    {analiseConflitos.length === 0 ? (
                      <Alert>
                        ...
                        <AlertDescription>
                          Parabéns! A grade está otimizada sem conflitos.
                        </AlertDescription>
                      </Alert>
                    ) : (
                      analiseConflitos.map((conflito, index) => (
                        <Alert key={index} variant="destructive" className="mb-2">
                          <AlertTriangle className="h-4 w-4" />
                          <AlertDescription>
                            <strong>{conflito.tipo === 'falta_horario' ? 'Falta de Horário' : 'Conflito de Profissional'}:</strong>
                            {conflito.tipo === 'falta_horario' ? (
                              <span> {conflito.profissional} - {conflito.crianca}: Necessário {conflito.necessario}, alocado {conflito.alocado}, faltante {conflito.faltante}</span>
                            ) : (
                              <span> {conflito.descricao} ({conflito.dia} às {conflito.horario})</span>
                            )}
                          </AlertDescription>
                        </Alert>
                      ))
